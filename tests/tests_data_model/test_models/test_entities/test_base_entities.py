...
class TestEntity:
    def test_entity_creation_with_suffix_only(self):
    ...
    def test_entity_creation_with_prefix_and_suffix(self):
    ...
    def test_invalid_prefix_patterns(self, invalid_prefix):
    ...
    def test_invalid_suffix_patterns(self, invalid_suffix):
        with pytest.raises(ValidationError):
...
class TestConceptEntity:
    ...
    def test_concept_entity_string_representation_with_version(self):
        entity = ConceptEntity(prefix="test", suffix="TestSuffix", version="1.0")
        assert str(entity) == "test:TestSuffix(version=1.0)"

    def test_concept_entity_without_version(self):
...
class TestUnknownEntity:
    def test_unknown_entity_creation(self):
        entity = UnknownEntity()
        assert entity.prefix == Undefined
        assert entity.suffix == Unknown

    def test_unknown_entity_id_property(self):
        entity = UnknownEntity()
        assert entity.id == str(Unknown)

    def test_unknown_entity_string_representation(self):
        entity = UnknownEntity()
...
class TestEntityComparison:
    ...
    def test_entity_ordering_different_types(self):
        entity = Entity(suffix="test")
        concept_entity = ConceptEntity(suffix="test")
