    ...
    def test_extract_triples(self) -> None:
        with monkeypatch_neat_client() as client:
            client.raw.rows.return_value = [
                Row(key="key1", columns={"column1": "value1", "column2": "100", "column3": "1983-01-22"}),
                Row(key="key2", columns={"column1": "value3", "column2": "200", "column3": "1983-01-22"}),
            ]
        extractor = RAWExtractor(client, "my_db", "my_table", str_to_ideal_type=True)
