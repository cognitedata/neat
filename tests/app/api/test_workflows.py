...
from tests.memory_cognite_client import MemoryClient
...
def test_rules(transformation_rules: Rules, fastapi_client: TestClient):
    ...
    response = fastapi_client.get("/api/rules", params={"workflow_name": "extract_rdf_graph_generate_assets"})
...
@pytest.mark.parametrize("workflow_name", ["extract_rdf_graph_generate_assets"])
def test_workflow_start(
    workflow_name: str, cognite_client: CogniteClient, fastapi_client: TestClient, data_regression, tmp_path
):
    # Arrange
    if workflow_name == "extract_rdf_graph_generate_assets":
        # When running this test in GitHub actions, you get permission issues with the default disk_store_dir.
        response = fastapi_client.get("/api/workflow/workflow-definition/extract_rdf_graph_generate_assets")
        definition = WorkflowDefinition(**response.json()["definition"])
        response = fastapi_client.post(
            "/api/workflow/workflow-definition/extract_rdf_graph_generate_assets", json=definition.model_dump()
        )
        assert response.status_code == 200
