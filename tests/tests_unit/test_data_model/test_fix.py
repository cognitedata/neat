...
class TestAutoGeneratedIdGeneration:
    def test_deterministic_and_valid_id_generation(self) -> None:
        ...
        assert AutoIdentifier.make(ref) == AutoIdentifier.make(ref)
        assert len(AutoIdentifier.make(ref)) <= AutoIdentifier.MAX_LENGTH

    def test_different_inputs_produce_different_ids(self) -> None:
        id1 = AutoIdentifier.make("VeryLongContainerOrPropertyNameThatRequiresApplyingHashing1")
        id2 = AutoIdentifier.make("VeryLongContainerOrPropertyNameThatRequiresApplyingHashing2")
        assert id1 != id2
