...
def input_rules_instances_iterator() -> Iterable[ParameterSet]:
    for cls_ in get_all_subclasses(UnvalidatedDataModel, only_concrete=True):
...
class TestInputRules:
    ...
    def test_issues_dump_load(self, input_rules: UnvalidatedDataModel) -> None:
...
def dataclass_to_parameters(
    input_rules_cls: type[UnvalidatedDataModel],
) -> dict[str, set[str]]:
