...
from cognite.neat.core._utils.producer_worker import ProducerWorkerExecutor
...
def setup_executor() -> tuple[
    ...
    executor: ProducerWorkerExecutor = ProducerWorkerExecutor(
        producer_iterable=download_iterable,
        process=process,
        work=write_to_file,
...
def test_run_no_processor() -> None:
    download_iterable = [[1, 2, 3], [4, 5, 6], [7, 8, 9]]
    write_to_file: MagicMock = MagicMock()
    ...
    assert write_to_file.call_count == len(download_iterable)
...
def test_download_worker_handles_full_queue(monkeypatch: Any) -> None:
    ...
    executor = ProducerWorkerExecutor(
        to_download,
        process=MagicMock(side_effect=process),
        work=MagicMock(side_effect=write_to_file),
        iteration_count=2,
...
def test_error_handling_in_download_worker() -> None:
    executor = ProducerWorkerExecutor(FailingIterator(), MagicMock(), 1, 10, process=MagicMock())
    executor.run()
