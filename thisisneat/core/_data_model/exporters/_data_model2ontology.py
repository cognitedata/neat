...
from thisisneat.core._constants import DEFAULT_NAMESPACE as NEAT_NAMESPACE
...
from thisisneat.core._data_model.models.data_types import DataType
from thisisneat.core._data_model.models.entities import ConceptEntity
from thisisneat.core._issues.errors import (
    PropertyDefinitionDuplicatedError,
)
from thisisneat.core._issues.warnings import PropertyDefinitionDuplicatedWarning
from thisisneat.core._utils.rdf_ import remove_namespace_from_uri
...
class OWLExporter(GraphExporter):
    ...
    def export(self, data_model: ConceptualDataModel) -> Graph:
        return Ontology.from_data_model(data_model).as_graph()
...
class SHACLExporter(GraphExporter):
    ...
    def export(self, data_model: ConceptualDataModel) -> Graph:
        return Constraints.from_data_model(data_model).as_graph()
...
class Ontology(_BaseConfig):
    ...
    def triples(self) -> list[tuple]:
        return list(self.as_graph())


class Constraints(_BaseConfig):
    """
    Represents a SHACL constraints. This class is used to generate a SHACL graph from conceptual data model.
...
class SHACLNodeShape(_BaseConfig):
    id_: URIRef
...
class SHACLPropertyShape(_BaseConfig):
    id_: BNode
    ...
    def from_property(
        ...
        return cls(
            id_=BNode(),
            path=(definition.instance_source[0] if definition.instance_source else namespace[definition.property_]),
