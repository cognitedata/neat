...
class ConfigureDefaultGraphStores(Step):
    ...
    def run(self, rules_data: RulesData | None = None) -> (FlowMessage, SourceGraph, SolutionGraph):  # type: ignore[override, syntax]
        ...
        source_store_dir = (
            Path(self.data_store_path) / Path(source_store_dir)
            if source_store_dir
            else None
        )
        ...
        if stores_to_configure in ["all", "source"]:
            if (
                source_store_type == stores.OxiGraphStore.rdf_store_type
                and "SourceGraph" in self.flow_context
            ):
                cast(
                    SourceGraph, self.flow_context["SourceGraph"]
                ).graph.upsert_prefixes(prefixes)
            ...
            if stores_to_configure == "source":
                return FlowMessage(
                    output_text="Source graph store configured successfully"
        ...
        if stores_to_configure in ["all", "solution"]:
            ...
            solution_store_dir = (
                self.data_store_path / Path(solution_store_dir)
            ...
            if (
                solution_store_type == stores.OxiGraphStore.rdf_store_type
                and "SolutionGraph" in self.flow_context
            ):
                cast(
                    SolutionGraph, self.flow_context["SolutionGraph"]
                ).graph.upsert_prefixes(prefixes)
                return FlowMessage(output_text="Stores already configured")
            ...
            if isinstance(solution_graph, stores.GraphDBStore):
                solution_graph.graph_db_rest_url = self.configs[
                    "solution_rdf_store.api_root_url"
                ]

            if stores_to_configure == "solution":
                return FlowMessage(
                    output_text="Solution graph store configured successfully"
...
class ResetGraphStores(Step):
    ...
    def run(self) -> FlowMessage:  # type: ignore[override, syntax]
        ...
        graph_store = cast(
            SourceGraph | SolutionGraph | None, self.flow_context.get(graph_name, None)
        )
...
class ConfigureGraphStore(Step):
    ...
    def run(  # type: ignore[override]
        ...
        store_dir = (
            ...
            if (value := self.configs["disk_store_dir"])
            else None
        )
        ...
        if (
            store_type == stores.OxiGraphStore.rdf_store_type
...
def reset_store(
    data_store_dir: Path | None, graph_store: stores.NeatGraphStoreBase | None = None
):
