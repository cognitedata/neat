...
from ._prune_graph import TwoHopFlattener, PruneDanglingNodes
...
class IODDTwoHopFlattener(TwoHopFlattener):
    ...
    def __init__(self):
        super().__init__(destination_node_type=IODD.TextObject,
                         property_predicate=IODD.value,
                         property_name="value")
...
class IODDPruneDanglingNodes(PruneDanglingNodes):
    _need_changes = frozenset(
        {
            str(IODDExtractor.__name__),
            str(IODDTwoHopFlattener.__name__)
        }
    )
