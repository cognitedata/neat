...
from cognite.neat.core._client.data_classes.instances import InstancesApplyResultList
from cognite.neat.core._client.utils._concurrency import NeatConcurrencySettings
from cognite.neat.core._client.utils._http_client import NeatRetryTracker
from cognite.neat.core._constants import DATA_MODELING_MAX_WRITE_WORKERS
...
class ExtendedInstancesAPI(InstancesAPI):
    def __init__(self, config: ClientConfig, api_version: str | None, cognite_client: CogniteClient) -> None:
        super().__init__(config, api_version, cognite_client)
        # The self._http_client retries 429 status codes, which is not desired when we want
        # to reduce the number of items sent in each request when a 400 error occurs.
    ...
    def apply_fast(
        ...
        Args:
            ...
            auto_create_start_nodes (bool): Whether to create missing start nodes for edges when ingesting. By default,
                the start node of an edge must exist before it can be ingested.
            auto_create_end_nodes (bool): Whether to create missing end nodes for edges when ingesting. By default,
            the end node of an edge must exist before it can be ingested.
            ...
            skip_on_version_conflict (bool): If existingVersion is specified on any of the nodes/edges in the input,
                the default behaviour is that the entire ingestion will fail when version conflicts occur. If
                    skipOnVersionConflict is set to true, items with version conflicts will be skipped instead. If no
                    version is specified for nodes/edges, it will do the writing directly.
            replace (bool): How do we behave when a property value exists? Do we replace all matching and existing
                values with the supplied values (true)? Or should we merge in new values for properties together with
                the existing values (false)? Note: This setting applies for all nodes or edges specified
                in the ingestion call.
        ...
        tasks = [
            (self._RESOURCE_PATH, task_items, NeatRetryTracker(self._http_client_with_retry.config))
        ...
        summary = execute_tasks(
            ...
            executor=NeatConcurrencySettings.get_data_modeling_write_executor(),
        )
    ...
    def _post_with_item_reduction_retry(
        ...
        retry_tracker: NeatRetryTracker,
    ) -> Response:
    ...
    def _sleep(self, retry_tracker: NeatRetryTracker, headers: MutableMapping[str, Any]) -> None:
