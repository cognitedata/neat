...
class DataModel(BaseModel):
    ...
    def from_rules(cls, transformation_rules: TransformationRules) -> Self:
        ...
        if not names_compliant:
            logging.error(exceptions.Error10(report=generate_exception_report(name_warnings)).message)
        ...
        if properties_redefined:
            logging.error(exceptions.Error11(report=generate_exception_report(redefinition_warnings)))
    ...
    def container_properties_from_dict(properties: dict[str, Property]) -> dict[str, ContainerProperty]:
        ...
        for property_id, property_definition in properties.items():
            ...
            # URIRef, i.e. edge
                ...
                logging.warning(exceptions.Warning60(property_id, property_definition.property_type).message)
                warnings.warn(
                    exceptions.Warning60(property_id, property_definition.property_type).message,
                    category=exceptions.Warning60,
                    stacklevel=2,
                )
    ...
    def view_properties_from_dict(
        ...
        for property_id, property_definition in properties.items():
            ...
            # edge 1-many
                ...
                logging.warning(exceptions.Warning61(property_id).message)
                warnings.warn(
                    exceptions.Warning61(property_id).message,
                    category=exceptions.Warning61,
                    stacklevel=2,
                )
    ...
    def to_cdf(self, client: CogniteClient):
        ...
        if existing_data_model or existing_containers or existing_views:
            raise exceptions.Error60(existing_data_model, existing_containers, existing_views)
    ...
    def find_existing_data_model(self, client: CogniteClient) -> set[str]:
        ...
        if model := client.data_modeling.data_models.retrieve((self.space, self.external_id, self.version)):
            ...
            logging.warning(exceptions.Warning64(self.external_id, self.version, self.space).message)
            warnings.warn(
                exceptions.Warning64(self.external_id, self.version, self.space).message,
                category=exceptions.Warning64,
                stacklevel=2,
            )
    ...
    def find_existing_containers(self, client: CogniteClient) -> set[str]:
        ...
        if existing_containers := set(self.containers.keys()).intersection(set(cdf_containers.keys())):
            logging.warning(exceptions.Warning62(existing_containers, self.space).message)
            warnings.warn(
                exceptions.Warning62(existing_containers, self.space).message,
                category=exceptions.Warning62,
                stacklevel=2,
            )
    ...
    def find_existing_views(self, client: CogniteClient) -> set[str]:
        ...
        if existing_views := set(self.views.keys()).intersection(set(cdf_views.keys())):
            logging.warning(exceptions.Warning63(existing_views, self.version, self.space).message)
            warnings.warn(
                exceptions.Warning63(existing_views, self.version, self.space).message,
                category=exceptions.Warning63,
                stacklevel=2,
            )
