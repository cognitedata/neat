...
def make_classes_compliant(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["Description"] = df["Description"].apply(lambda x: x[:1028] if isinstance(x, str) else None)

    # Add missing parent classes to the dataframe
    df = pd.concat(
        [df, pd.DataFrame(_add_parent_class(df))],
        ignore_index=True,
    )
...
def _object_property_class() -> dict:
    return {
        ...
        "Match Type": "exact",
        "Comment": "Based on owl:ObjectProperty but adapted to NEAT and use in CDF.",
    }
...
def _data_type_property_class() -> dict:
    return {
        ...
        "Match Type": "exact",
        "Comment": "Based on owl:DatatypeProperty but adapted to NEAT and use in CDF.",
    }
