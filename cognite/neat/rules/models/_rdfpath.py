...
from cognite.neat.rules.issues.tables import NotValidRAWLookUp, NotValidRDFPath, NotValidTableLookUp
...
def parse_traversal(raw: str) -> SelfReferenceProperty | SingleProperty | Hop:
    if result := CLASS_ID_REGEX_COMPILED.match(raw):
        ...
        raise NotValidRDFPath(raw).as_pydantic_exception()
...
def parse_table_lookup(raw: str) -> TableLookup:
    ...
    raise NotValidTableLookUp(raw).as_pydantic_exception()
    ...
    match rule_type:
        ...
        case TransformationRuleType.rawlookup:
            ...
            if Counter(rule_raw).get("|") != 1:
                raise NotValidRAWLookUp(rule_raw).as_pydantic_exception()
