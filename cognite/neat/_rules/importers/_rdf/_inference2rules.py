...
class SubclassInferenceImporter(BaseRDFImporter):
    ...
    def _to_rules_components(
        ...
        Returns:
            Tuple of data model and prefixes of the graph
        """
        count_by_class_subclass_pair: dict[tuple[URIRef, str], int] = {}
        # Infers all the classes in the graph
        for result_row in self.graph.query(self._ordered_subclass_query):
            class_uri, subclass_literal, instance_count = cast(tuple[URIRef, RdfLiteral, RdfLiteral], result_row)
            count_by_class_subclass_pair[(class_uri, str(subclass_literal.toPython()))] = instance_count.toPython()

        for (class_uri, subclass), instance_count in count_by_class_subclass_pair.items():
            property_query = self._properties_query.format(type=class_uri, subtype=subclass, neat_type=NEAT.type)
            for result_row in self.graph.query(property_query):
                ...
                occurrence_query = self._max_occurrence_query.format(
                    type=class_uri, subtype=subclass, property=property_uri, neat_type=NEAT.type
                )
                max_occurrence_literal, *_ = cast(tuple[RdfLiteral, Any], list(self.graph.query(occurrence_query)))
        ...
        #     "classes": [cls.dump(self._rules.metadata.space) for cls in classes.values()],
        #     "properties": list(properties.values()),
        #     "prefixes": prefixes,
        # }
