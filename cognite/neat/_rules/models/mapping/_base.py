...
class Mapping(BaseModel, Generic[T_Source, T_Destination]):
    source: T_Source
    destination: T_Destination


class MappingList(list, MutableSequence[Mapping[T_Source, T_Destination]]):
    @classmethod
    ...
    def as_destination_by_source(self) -> dict[T_Source, T_Destination]:
    ...
    def __iter__(self) -> Iterator[Mapping[T_Source, T_Destination]]:
        return super().__iter__()

    @overload
    def __getitem__(self, index: SupportsIndex) -> Mapping[T_Source, T_Destination]: ...

    @overload
    def __getitem__(self, index: slice) -> "MappingList[T_Source, T_Destination]": ...

    def __getitem__(
        self, index: SupportsIndex | slice, /
    ) -> "Mapping[T_Source, T_Destination] | MappingList[T_Source, T_Destination]":
        if isinstance(index, slice):
            return MappingList[T_Source, T_Destination](super().__getitem__(index))
...
class RuleMapping(BaseModel):
    ...
    def as_mapping_list(cls, value: Sequence[Any], info: ValidationInfo) -> Any:
        if isinstance(value, Sequence) and not isinstance(value, MappingList):
            ...
            source_cls, dest_cls = get_args(annotation)
            return annotation([Mapping[source_cls, dest_cls].model_validate(item) for item in value])  # type: ignore[valid-type, index]
        return value
