...
class ExploreAPI:
    ...
    def types(self) -> pd.DataFrame:
        ...
        return pd.DataFrame(self._state.instances.store.queries.read.types_with_instance_and_property_count())

    def properties(self) -> pd.DataFrame:
        """List all the properties of a type of instances in the session."""
        return pd.DataFrame(self._state.instances.store.queries.read.properties_with_count())

    def instance_with_properties(self, type: str) -> dict[str, set[str]]:
        """List all the instances of a type with their properties."""
        available_types = self._state.instances.store.queries.read.list_types(remove_namespace=False)
        ...
        return self._state.instances.store.queries.read.instances_with_properties(type_uri, remove_namespace=True)
